...
void loop()
{
  ...
  if (currentTime>totalTime)  //*4 to hop 4x
  {
//    while(true);////////////////// CALLEN (to run only once)
  ...
  //try - print out commanded angles for each iteration. current time greater than total time in trajectory given then things will be weird. 
  //remove while(true) change Hz to run at 1Hz but not on line 136. data sent to servo must be delayed. check current loop time
